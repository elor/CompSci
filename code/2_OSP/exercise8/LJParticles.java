...
public class LJParticles implements Drawable, ODE {
  ...
  public void computeAcceleration() {
    ...
    for (int i = 0; i < N - 1; i++) {
      for (int j = i + 1; j < N; j++) {
        ...
        ax[j] -= fx;
        ay[j] -= fy;
        totalPotentialEnergyCurrent += 4.0 * (oneOverR6 * oneOverR6 - oneOverR6);
        virialCurrent += dx * fx + dy * fy;
      }
    }
    totalPotentialEnergyAccumulator += totalPotentialEnergyCurrent;
    virialAccumulator += virialCurrent;
  }
   ...
   * 
   * Errata: On page 276 in Eq. 8.11, after the equal sign, the first appearance
   * of N should be 1/N and in Eq. 8.12 after the minus sign 1/N should be N. We
   * thank Mike Cooke for pointing out this error.
   * 
   * @return double
   */
  public double getHeatCapacity() {
    double meanTemperature = getMeanTemperature();
    double meanKineticEnergySquared = totalKineticEnergySquaredAccumulator
        / steps;
    double meanKineticEnergy = totalKineticEnergyAccumulator / steps;
    // heat capacity related to fluctuations of kinetic energy
    double sigma2 = meanKineticEnergySquared - meanKineticEnergy
        * meanKineticEnergy;
  ...
  public void setVelocities() {
    ...
    double v2sum = 0; // rescale velocities to obtain desired initial kinetic
                      // energy
    for (int i = 0; i < N; ++i) {
  ...
  public void step() {
    ...
    totalKineticEnergyCurrent = 0;
    for (int i = 0; i < N; i++) {
      totalKineticEnergyCurrent += (state[4 * i + 1] * state[4 * i + 1] + state[4 * i + 3]
          * state[4 * i + 3]);
      state[4 * i] = pbcPosition(state[4 * i], Lx);
      state[4 * i + 2] = pbcPosition(state[4 * i + 2], Ly);
    }
    totalKineticEnergyCurrent *= 0.5;
    steps++;
    totalKineticEnergyAccumulator += totalKineticEnergyCurrent;
    totalKineticEnergySquaredAccumulator += totalKineticEnergyCurrent
        * totalKineticEnergyCurrent;
    t += dt;
  }
