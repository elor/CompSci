...
/**
 * PreferentialAttachmentModel implements the preferential attachment model for
 * networks.
 * 
 * @author Jan Tobochnik, Wolfgang Christiann, Harvey Gould
 * @version 1.0 revised 06/24/05
 */
public class PreferentialAttachment2 implements Drawable {
	...
	public int n = 0; // current number of nodes
	...
	public void initialize() {
		degree = new int[N]; // degree distribution to be averaged over many
								// networks
	...
	public double coeffi() {
		...
		int y = 0;
		for (int i = 0; i < n; i++) {
			for (int e = 0; e < node[i]; e++) {
				int j = linkFrom[i * m + e];

				for (int f = 0; f < node[i]; f++) {
					if (e == f) {
						continue;
					}

					boolean open = true;
					int k = linkFrom[i * m + f];
					for (int g = 0; g < node[k]; g++) {
						int l = linkFrom[k * m + g];
						if (l == j) {
							open = false;
							break;
						}
	...
	public void setPosition(int i) {
		double r2min = 1000. / N;
		boolean ok = true; // used to insure two nodes are not drawn too close
							// to each other
		do {
			ok = true;
			x[i] = Math.random() * 100;
	...
	public void step() {
		...
		// for (int i = 0; i < n; i++) { // accumulate data for degree
		// // distribution
		// degree[node[i]]++;
		// }
