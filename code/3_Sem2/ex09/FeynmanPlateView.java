...
/**
 * FeynmanPlateView shows a rotating plate as seen from the space (laboratory)
 * frame.
 * 
...
public class FeynmanPlateView {
  ...
  /**
   * Constucts space view for a rotating plate. A space view shows the rigid
   * body as seen from the intertial laboratory frame.
   * 
   * @param rigidBody
   *          RigidBody
   */
  public FeynmanPlateView(FeynmanPlate rigidBody) {
    ...
    frame.setAllowQuickRedraw(false); // The scene is simple, so draw it
                                      // properly when rotating
   ...
   * @param isBox
   */
  void resetPlate(boolean isBox) {
    if (plate != null) {
      bodyGroup.removeElement(plate);
    }
    plate = isBox ? new ElementBox() : new ElementCylinder();
  ...
  void update() {
    ...
    omega.setSizeXYZ(s * vec[0], s * vec[1], s * vec[2]); // scale omega to same
                                                          // length as angular
                                                          // momentum
