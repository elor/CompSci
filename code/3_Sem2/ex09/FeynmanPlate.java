...
/**
 * RigidBody models rigid body dynamics by solving Euler's equations using an
 * ODE solver.
 * 
...
public class FeynmanPlate {
  ...
  void setOrientation(double[] q) {
    double norm = Math.sqrt(q[0] * q[0] + q[1] * q[1] + q[2] * q[2] + q[3]
        * q[3]);
    q0 = q[0] / norm;
    q1 = q[1] / norm;
    q2 = q[2] / norm;
  ...
  public void advanceTime() {
    ...
    q2 += q2dot * dt;
    q3 += q3dot * dt;
    double norm = 1 / Math.sqrt(q0 * q0 + q1 * q1 + q2 * q2 + q3 * q3); // normalize
                                                                        // to
                                                                        // eliminate
                                                                        // drift
    q0 *= norm;
